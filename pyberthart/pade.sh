...
do
   ...
   do
	echo $gamma $dw
	python /home/redo/BERTHA/Exanalysis/misc/pade/pade_transform.py --limit 25000 --gamma $gamma -f ./omp.mklserial.quad.2/dipole.txt --dw $dw --fmin 2.5 --frequency 6.5 -o zz_w.txt > pade.txt
...
do
   ...
   do
	echo $gamma $dw
	python /home/redo/BERTHA/Exanalysis/misc/pade/pade_transform.py --limit 25000 --gamma $gamma -f ./omp.mklparallel.quad.2/dipole.txt --dw $dw --fmin 2.5 --frequency 6.5 -o zz_w.txt > pade.txt
	sed "s/dw/$dw/" plot.p > tmp
	sed "s/gamma/$gamma/" tmp > plot.p$n
	gnuplot < plot.p$n
	mv 1.pdf  omp_mklserial_quad_2_"$n".pdf
