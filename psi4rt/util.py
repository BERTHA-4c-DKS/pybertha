...
class Molecule():
  ...
  def __init__(self,fgeom='geomA.xyz',label=False):
  ...
  def core_geom(self,idx1,idx0=0):
         ...
         for m in tmp[idx0:idx1]:
            res += m+'\n'
         return res
def set_input(fgeomA,fgeomB,charge):
  molA = Molecule(fgeomA,label=True)
  molA.set_charge(charge)
  ...
  mol = psi4.geometry(molA.geometry())
  
  return molA.geometry(),speclist,mol
