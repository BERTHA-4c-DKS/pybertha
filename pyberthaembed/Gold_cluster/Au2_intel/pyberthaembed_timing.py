...
def runspberthaembed (pberthaopt, restart = False, stdoutprint = True):
    ...
    for out_iter in range (maxiter):
        ...
        time_bertharun_start = time.time()
        process_time_bertharun_start = time.process_time()

        ovapm, eigem, fockm, eigen = bertha.run(eigem)
        etotal2 = bertha.get_etotal()

        time_bertharun_end= time.time()
        process_time_bertharun_end= time.process_time()


        print("Bertha.run including embedd (pp calc)     : %15.5f"%(time_bertharun_end - time_bertharun_start), \
                    " (CPU time: %15.5f"%(process_time_bertharun_end - process_time_bertharun_start), ") s ")
