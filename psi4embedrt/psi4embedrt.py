...
os.environ['RTHOME'] = "/home/mdesantis/pybertha/psi4rt"
...
if args.fde:
  ...
  file_h2o = os.path.join (WD, geomA)
  file_nh3 = os.path.join (WD, geomB)

  m_h2o = pyadf.molecule(file_h2o)
  ...
  r_isolated_nh3 = pyadf.adfsinglepointjob(m_nh3, basis_active, settings=adf_settings, options=['NOSYMFIT']).run()
  if args.grid_opts == 1:
      frags = [ pyadf.fragment(None,  [m_h2o]),
              pyadf.fragment(r_isolated_nh3, [m_nh3], isfrozen=True) ]
  ...
  if args.grid_opts == 3:
      r_act = pyadf.adfsinglepointjob(m_h2o, basis_active, settings=adf_settings, options=['NOSYMFIT']).run()
...
if args.fde:
  ...
  density_h2o = GridFunctionContainer([dens_gf, densgrad, denshess])
  end_map=time.time()
  ...
  #grid_h2o, isolated_elpot_h2o, density_h2o = \
  #    GridFunctionReader.read_density_elpot_xyzwv(os.path.join("./", 'FRZDNS.a.txt'))
  #nel_h2o = density_h2o[0].integral()

  #check that grid_h2o and agrid should be the same

  #GridWriter.write_xyzw(grid=grid_h2o,filename=os.path.join("./", 'ADFGRID_H20'),add_comment=False)

  #isolated ammonia get densities and nuclear potentials



  isolated_dens_nh3  = r_isolated_nh3.get_density(grid=agrid, fit=False, order=2) #grid=grid_h2o=agrid
  isolated_vnuc_nh3  = r_isolated_nh3.get_potential(grid=agrid, pot='nuc')
  isolated_coul_nh3  = r_isolated_nh3.get_potential(grid=agrid, pot='coul')
  isolated_elpot_nh3 = isolated_vnuc_nh3 + isolated_coul_nh3


  print("1. Evaluation of the embedding potential in two steps:")
  print ("   a. getting the non-additive potential")
  
  #nadkin_h2o_dd = embed_eval.get_nad_pot_kin(density_h2o,isolated_dens_nh3)
  #nadxc_h2o_dd  = embed_eval.get_nad_pot_xc(density_h2o, isolated_dens_nh3)
  #embpot_h2o_dd = nadkin_h2o_dd+nadxc_h2o_dd+isolated_elpot_nh3



  nadpot_h2o = embed_eval.get_nad_pot(density_h2o, isolated_dens_nh3)
  #nadpot_nh3 = embed_eval.get_nad_pot(isolated_dens_nh3, density_h2o) #not used if the density of environment is not propagated

  print("   b. adding the electrostatic potential")

  embpot_A_2 = isolated_elpot_nh3 + nadpot_h2o
  if args.fdecorr : 
   embpot_A_2 = fde_util.fcorr(embpot_A_2,density_h2o[0],isolated_dens_nh3[0])
  #embpot_B_2 = isolated_elpot_h2o + nadpot_nh3 #not used if the density of environment is not propagated
